...
SELECT cte.dt as day, COALESCE(daily_sums.s, 0) / COALESCE(puPerDay.c, 1) as arppu
    FROM cte
    LEFT JOIN (SELECT CAST(payment_dttm AS DATE) AS dt, SUM(payment_sum) as s FROM payments GROUP BY dt)
      AS daily_sums ON cte.dt = daily_sums.dt
    LEFT JOIN (SELECT CAST(payment_dttm AS DATE) AS dt, COUNT(DISTINCT  payments.user_id) as c FROM payments GROUP BY dt)
